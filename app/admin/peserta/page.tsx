...
export default function PesertaPage() {
  ...
  function openEdit(p: Peserta) {
    ...
    setForm({
      noUrut: p.noUrut,
      nama: p.nama,
      laguWajib: p.laguWajib,
      laguBebas: p.laguBebas,
      asalSkpd: p.asalSkpd,
    });
    setFoto(null);
    setPreview(p.fotoPath);
    setError(null);
    setModal(true);
  }
  ...
  async function save() {
    ...
    const url = editId ? `/api/admin/peserta/${editId}` : "/api/admin/peserta";
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <button
          onClick={openAdd}
          className="rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-bold shadow-lg shadow-brand-600/30 active:scale-95"
        >
          + Tambah Peserta
        </button>
      </div>
          ...
          <div className="max-h-[90vh] w-full max-w-md animate-pop-in overflow-y-auto rounded-3xl bg-night-800 p-6 ring-1 ring-white/10 scrollbar-thin">
            ...
            <div className="mt-4 flex gap-2">
              <button
                ...
                disabled={saving}
                className="flex-1 rounded-xl bg-white/5 py-3 font-semibold text-white/70 ring-1 ring-white/10"
              >
                Batal
              </button>
              <button
                onClick={save}
                disabled={saving}
                className="flex-1 rounded-xl bg-brand-600 py-3 font-bold disabled:opacity-60"
              >
